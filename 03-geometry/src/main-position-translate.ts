...
const movePlane = {
  moveX: plane.position.x,
  moveY: plane.position.y,
  moveZ: plane.position.z,
}


// 设置 lil-gui，控制 plane 的平移
const gui = new GUI()
gui.add(movePlane, 'moveX').min(-10).max(10).step(0.01).onChange((value) => {
  // plane.position.x = value
  planeGeometry.translate(value, 0, 0)
})
